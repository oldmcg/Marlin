...
#include "MarlinConfig.h"
  ...
  int    unified_bed_leveling::g29_verbose_level,     // V
         unified_bed_leveling::g29_phase_value,       // P
         unified_bed_leveling::g29_repetition_cnt,    // R
         unified_bed_leveling::g29_storage_slot = 0,  // S or L
         unified_bed_leveling::g29_map_type,          // T
         unified_bed_leveling::g29_stow_each,         // E
         unified_bed_leveling::g29_grid_size;         // J
   ...
   */
  void unified_bed_leveling::probe_entire_mesh(const float &lx, const float &ly, const bool do_ubl_mesh_map, bool close_or_far) {
    ...
    do {
      ...
      if (location.x_index >= 0) {    // mesh point found and is reachable by probe
        ...
        const float measured_z = probe_pt(LOGICAL_X_POSITION(rawx), LOGICAL_Y_POSITION(rawy), g29_stow_each, g29_verbose_level); // TODO: Needs error handling
    ...
    void unified_bed_leveling::tilt_mesh_based_on_probed_grid() {
      ...
      for (uint8_t ix = 0; ix < g29_grid_size; ix++) {
        ...
        for (int8_t iy = 0; iy < g29_grid_size; iy++) {
          ...
          float measured_z = probe_pt(LOGICAL_X_POSITION(x), LOGICAL_Y_POSITION(y), g29_stow_each, g29_verbose_level);
          #if ENABLED(DEBUG_LEVELING_FEATURE)
            if (DEBUGGING(LEVELING)) {
              SERIAL_CHAR('(');
  ...
  #else // ENABLED(DELTA), round bed

    void unified_bed_leveling::tilt_mesh_based_on_probed_circle() {

      struct linear_fit_data lsf_results;
      incremental_LSF_reset(&lsf_results);

      // For delta, instead of grid, probe a circle at DELTA_PROBEABLE_RADIUS,
      // aligned to the towers.  If more than 9 points, also probe at radius/2,
      // with 1/3 of points, and if not multiple of 3 also probe center point.
      //    3: radius at towers
      //    4: radius at towers plus center
      //    6: radius at towers and between towers
      //    7: ..and center
      //    9: radius at towers and between towers and radius/2 at towers
      //       ..etc..

      uint8_t triplets = g29_grid_size >= 3 ? g29_grid_size / 3 : 1,
              center   = g29_grid_size % 3,
              inner    = 3 * (triplets / 3),
              outer    = (3 * triplets) - inner;

      for (uint8_t ring = 2; ring; ring-- ) {
  ...
  void unified_bed_leveling::apply_tilt_to_mesh(struct linear_fit_data &lsf_results) {
    ...
    if (g29_verbose_level > 3) {
      ...
      SERIAL_ECHOPGM("  N=");
      SERIAL_PROTOCOL_F(lsf_results.N, 0);
      SERIAL_EOL();
    }
