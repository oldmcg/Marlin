...
#include "MarlinConfig.h"
  ...
  void unified_bed_leveling::line_to_destination_cartesian(const float &feed_rate, uint8_t extruder) {
    ...
    inline void _O3 ubl_buffer_segment_raw( float rx, float ry, float rz, float le, float fr ) {
      ...
      #elif IS_SCARA  // apply scara kinematics (should be changed to save raw->logical->raw)
        ...
        // Cartesian _buffer_line takes LOGICAL, not RAW coordinates
    ...
    bool _O3 unified_bed_leveling::prepare_segmented_line_to(const float ltarget[XYZE], const float &feedrate) {
