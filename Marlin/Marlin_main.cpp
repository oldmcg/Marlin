...
static void clean_up_after_endstop_or_probe_move() {
  ...
  inline bool is_bed_leveling_enabled() {
  ...
  void set_bed_leveling_enabled(bool enable/*=true*/) {
      ...
      #if PLANNER_LEVELING
        if (ubl.state.active != enable) {               // changing state
          if (ubl.state.active) {                       // leveling from on to off
            // change unleveled (virtual) current_position to physical current_position without moving steppers.
            planner.apply_leveling(current_position[X_AXIS], current_position[Y_AXIS], current_position[Z_AXIS]);
            ubl.state.active = false;                   // disable only AFTER calling apply_leveling
          }
          else {                                        // leveling from off to on
            ubl.state.active = true;                    // enable BEFORE calling unapply_leveling, otherwise ignored
            // change physical current_position to unleveled (virtual) current_position without moving steppers.
    ...
    void set_z_fade_height(const float zfh) {
        ...
        const bool level_active = is_bed_leveling_enabled();
        if (level_active) {
      ...
      #else // previous non-UBL code

        planner.z_fade_height = zfh;
        planner.inverse_z_fade_height = RECIPROCAL(zfh);

        if (is_bed_leveling_enabled()) {
          set_current_from_steppers_for_axis(
            #if ABL_PLANAR
              ALL_AXES
            #else
...
void home_all_axes() { gcode_G28(true); }
  ...
  inline void gcode_G30() {
    ...
    inline void gcode_G33() {
      ...
      const bool stow_after_each = code_seen('E') ? code_value_bool() : true;   // default preserves original behavior
      ...
      home_offset[Z_AXIS] -= probe_pt(0.0, 0.0, stow_after_each, 1); // 1st probe to set height
      do_probe_raise(Z_CLEARANCE_BETWEEN_PROBES);
      ...
      do {
        ...
        if (_7p_calibration) { // probe extra center points
          for (int8_t axis = _7p_multi_circle ? 11 : 9; axis > 0; axis -= _7p_multi_circle ? 2 : 4) {
            ...
            z_at_pt[0] += probe_pt(cos(a) * r, sin(a) * r, stow_after_each, 1); // TODO: Needs error handling
          }
        ...
        if (!_1p_calibration) {  // probe the radius
          ...
          for (uint8_t axis = start; axis < 13; axis += step) {
            ...
            for (float circles = -offset_circles ; circles <= offset_circles; circles++) {
              ...
              z_at_pt[axis] += probe_pt(cos(a) * r, sin(a) * r, stow_after_each, 1); // TODO: Needs error handling
            }
            zig_zag = !zig_zag;
